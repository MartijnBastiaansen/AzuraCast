...
const { data: props, isLoading: propsLoading } =
    useQuery<ApiAdminVueCustomFieldProps>({
        queryKey: [QueryKeys.AdminCustomFields, "props"],
        queryFn: async ({ signal }) => {
            const { data } = await axios.get<ApiAdminVueCustomFieldProps>(
                propsUrl.value,
                { signal },
            );
            return data;
...
const itemProvider = useApiItemProvider(listUrl, [
...
const { doDelete } = useConfirmAndDelete($gettext("Delete Custom Field?"), () =>
    relist(),
);
</script>
